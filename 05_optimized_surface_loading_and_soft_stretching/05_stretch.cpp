...
bool loadMedia()
{
    ...
    gKeyPressSurface[KEY_PRESS_SURFACE_UP] = loadSurface("../04_key_presses/up.bmp");
    if (gKeyPressSurface[KEY_PRESS_SURFACE_UP] == NULL) {
        MessageBox(NULL, "Failed to load up image", "Error", MB_OK);
        return false;
    }

    gKeyPressSurface[KEY_PRESS_SURFACE_DOWN] = loadSurface("../04_key_presses/down.bmp");
    if (gKeyPressSurface[KEY_PRESS_SURFACE_DOWN] == NULL) {
        MessageBox(NULL, "Failed to load down image", "Error", MB_OK);
        return false;
    }

    gKeyPressSurface[KEY_PRESS_SURFACE_LEFT] = loadSurface("../04_key_presses/left.bmp");
    if (gKeyPressSurface[KEY_PRESS_SURFACE_LEFT] == NULL) {
        MessageBox(NULL, "Failed to load left image", "Error", MB_OK);
        return false;
    }

    gKeyPressSurface[KEY_PRESS_SURFACE_RIGHT] = loadSurface("../04_key_presses/right.bmp");
